#!/usr/bin/env bash
# glpi-restore-application-data.sh [backup-file-name]
#
# Replaces GLPI's application data with one of the archives the backups
# service wrote.
#
#   ./glpi-restore-application-data.sh               list and ask
#   ./glpi-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using GLPI." >&2
  exit 1
fi
