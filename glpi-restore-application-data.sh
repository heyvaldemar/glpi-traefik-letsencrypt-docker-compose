...
BACKUP_PATH="/srv/glpi-application-data/backups/"
RESTORE_PATH="/var/www/html/glpi/"
BACKUP_PREFIX="glpi-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$GLPI_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$GLPI_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
